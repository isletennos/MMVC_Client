...
class Hyperparameters():
    ...
    def audio_trans(self, tdbm, input, net_g, noise_data):
        ...
        with torch.no_grad():
            ...
            _, _, spec, spec_lengths, _, _, sid_src = [x for x in data]
    ...
    def vc_run(self):
        ...
        try:
            ...
            trance_data_A = self.audio_trans(tdbm, in_raw_data_A, net_g, noise_data)
            ...
            while True:
                ...
                in_raw_data_B = audio_input_stream.read(Hyperparameters.DELAY_FLAMES, exception_on_overflow = False)
                back_in_raw_data_B = back_audio_input_stream.read(Hyperparameters.DELAY_FLAMES, exception_on_overflow = False)
                ...
                #ボイチェン
                #trancedataのsizeは(frame_length*2)となっている type=byte 16384
                trance_data_B = self.audio_trans(tdbm, in_raw_data_A, net_g, noise_data)
                #overlap + 後半部分のみ使う
